...
function ENT:AddSound(snd)
	if #self.SoundQueue < self.SoundLimit and self.maxSounds >= self.soundsPlayedLS then
		self.soundsPlayedLS = self.soundsPlayedLS + 1
...
local nextplay = 0
local useDecrement = CurTime() + 1
---
-- @realm shared
function ENT:Think()
	if (CurTime() >= useDecrement) and (self.soundsPlayedLS >= 1) then
		self.soundsPlayedLS = self.soundsPlayedLS - 1
		useDecrement = CurTime() + 2
	end
